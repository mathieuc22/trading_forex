...
def create_currency():
    ...
    for idx, currency in enumerate(currencies, 1):
        ...
        change_p = currency.find("td", class_=f"pid-{idx}-pcp").text
        time = currency.find("td", class_=f"pid-{idx}-time").text
        formattime = datetime.datetime.strptime(time, "%d/%m").strftime("2020-%m-%d")
...
def update_currency():
    ...
    for idx, currency in enumerate(currencies, 1):
        ...
        # create dictionary
        data = {'pair':pair, 'bid':bid, 'ask':ask, 'high':high, 'low':low, 'change':change, 'change_p':change_p, 'time':formattime}
